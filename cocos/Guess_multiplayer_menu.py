...
class MainMenu(Menu):
    def __init__(self):
        ...
        start_item=ImageMenuItem('IMG/24_1.png',self.on_start_item_callback)
        multi_item=ImageMenuItem('IMG/24_2.png',self.on_multi_item_callback)
        ...
        self.create_menu([start_item,multi_item,rule_item,exit_item],layout_strategy=fixedPositionMenuLayout([(100,380),(300,380),(500,380),(700,380)]))

    def on_start_item_callback(self):
        print("on_start_item_callback")
        scene = Guess_multiplayer_game.create_scene()
        director.replace(scene)

    def on_multi_item_callback(self):
        print("on_multi_item_callback")
        scene = Guess_multiplayer_game.create_scene()
        director.replace(scene)
