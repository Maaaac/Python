...
class Guess_menu(Layer):
    ...
    def __init__(self):
        ...
        lable3 = Sprite("IMG/30.png")
        lable3.position = 600,100
        self.add(lable3,-1)
        ...
        text4 = Label(u"若无法输入 请尝试切换 输入法为英文 输入模式",
            font_name = 'Times New Roman',
            font_size = 20,
            anchor_x = 'center',anchor_y = 'center',color=(7,7,14,200),width = 8,multiline = True)
        text4.position=550, 105
        self.add(text4,1)
    ...
    def on_key_press(self, k, m):                      #键盘事件，内部执行猜数列和特殊指令的语句
        if self.text1.element.text[4:] =='  4A0B   you win and press malicious arrest exit':
            exit()
        else:
            if k == pyglet.window.key.ENTER:
                ...
                if self.keys_pressed.decode().isdecimal():                #判断输入的字符串内容是否全为数字
                    if len(self.keys_pressed) == 4:
                        if self.keys_pressed == self.anw:
                            self.text1.element.text = self.keys_pressed+'  4A0B   you win and press malicious arrest exit'
                        else:
                            B_COUNT = 0
                            for i in range(4):
                                for j in range(4):
            ...
            #self.text1.element.text = self.text1.element.text + '\n'+self.keys_pressed
                ...
                elif self.keys_pressed == 'SELECTANSWER':        #查询
                    self.text1.element.text = str(self.anw)

                elif self.keys_pressed == 'RE':                  #重置
                    self.anw =self.random_num()
                    self.text1.element.text = u"成功重置答案"
                    self.text2.element.text = str(self.anw)                

                elif self.keys_pressed[0:10] == 'MODIFIEDTO':         #修改
